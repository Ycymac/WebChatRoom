...
export default {
  ...
  methods: {
    ...
    /* async pollGroupMessages() {
      try {
        const res = await axios.post('http://localhost:8081/groupmsg/poll', {
          groupId: this.selectedGroup.id
        });
        ...
        if (res.data.code === 200 && res.data.data) {
          ...
          // 检查是否有新成员加入
          const currentMembers = this.groupMembers[this.selectedGroup.id] || {};
          const newMemberIds = [];

          newMessages.forEach(msg => {
            if (!currentMembers[msg.sender_id]) {
              newMemberIds.push(msg.sender_id);
            }
          });
      ...
      while (this.pollingActive) {
        try {
          const response = await axios.post('http://localhost:8081/groupmsg/poll', {
            groupId: this.selectedGroup ? this.selectedGroup.id : 0
          }, {
            timeout: 30000
          });
          ...
          if (response.data.code === 200 && response.data.data && response.data.data.length) {
            ...
            newMessages.forEach(newMsg => {
              // **情况一：这是我发送消息的回声**
              // 判断依据：消息是我发的，并且它带有一个 clientMsgId
              console.log("当前有消息，进行消息发送者判断："+newMsg.messageContent);
              if (newMsg.senderId === this.myId && newMsg.clientMsgId) {
                const index = this.messages.findIndex(m => m.clientMsgId === newMsg.clientMsgId);
                // 如果在数组中找到了对应的临时消息
                if (index !== -1) {
                  // 就用轮询返回的完整消息替换掉它
                  this.$set(this.messages, index, { ...newMsg, status: 'sent' });
                }
                // 如果没找到（极罕见的边缘情况），则检查ID不重复再添加
                else if (!this.messages.some(m => m.id === newMsg.id)) {
              ...
              // **情况二：这是别人的新消息**
                ...
                if (!this.messages.some(m => m.id === newMsg.id)) {
                  console.log("群聊中他人发送的新消息："+newMsg.messageContent);
                  this.messages.push(newMsg);
                }
              }
            });
            ...
            // 更新UI（滚动和未读数）的逻辑可以保持
            const isCurrentGroupMsg = newMessages.some(m => this.selectedGroup && m.groupId === this.selectedGroup.id);
            if (isCurrentGroupMsg) {
              this.smartScrollToBottom();
            }

            newMessages.forEach(msg => {
              if (!this.selectedGroup || msg.groupId !== this.selectedGroup.id) {
                if (!this.unreadCounts[msg.groupId]) {
                  this.$set(this.unreadCounts, msg.groupId, 0);
                }
                this.unreadCounts[msg.groupId]++;
              }
            });
